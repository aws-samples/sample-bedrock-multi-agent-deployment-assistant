...
```
process_design_task()
  └── update_task_status(COMPLETED) → local JSON file
...
| Gap | Why Acceptable |
|---|---|
| No local SQS FIFO deduplication | Local queue.Queue is single-threaded; double-submit can't happen |
| No local DynamoDB Stream | notify_fn callback achieves the same result synchronously |
| No local EventBridge Pipe filtering | notify_fn is only called on terminal states (same filter semantics) |
| No local heartbeat cleanup | Local /ws connections are in-memory; no stale connection issue |
| No local Cognito auth | tenant_id from query param; security testing uses staging AWS |
| No local CloudWatch metrics | Structured JSON logs to stdout provide equivalent observability |
